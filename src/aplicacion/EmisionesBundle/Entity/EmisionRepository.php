...
class EmisionRepository extends EntityRepository
{
    public function TotalTipo(\DateTime $inicio=null, \DateTime $fin=null,$estado=null, $counter=null)
    {
        ...
        $qb->select('count(emision.id)');
        $qb->from('EmisionesBundle:Emision','emision');
        $qb->andWhere('emision.fecha between :inicio AND :fin');
        if(is_null($inicio))
        {
        ...
        if(!is_null($estado))
        {
           $qb->andWhere('emision.estado = :estado');
           $qb->setParameter('estado', $estado); 
        }
        if(!is_null($counter))
        {
           $qb->andWhere('emision.usuario = :counter');
